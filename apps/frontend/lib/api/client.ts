...
const INTERNAL_API_ORIGIN = process.env.BACKEND_ORIGIN || 'http://127.0.0.1:8002';
 ...
 * @param options - Standard RequestInit options (pass { signal } for external abort control)
 * @param timeoutMs - Optional request timeout in milliseconds (default: 20_000)
 */
export async function apiFetch(
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      ...
      throw new Error(
        'Request timed out. Please try again with a shorter job description or check your connection.'
      );
    }
    throw error;
  } finally {
