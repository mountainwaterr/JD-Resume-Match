...
async def upload_resume(file: UploadFile = File(...)) -> ResumeUploadResponse:
    ...
    # Detect resume language for auto language matching in modification
    detected_lang = detect_language(markdown_content)
    logger.info(f"Detected resume language: {detected_lang}")
    ...
    db.update_resume(resume["resume_id"], {"language": detected_lang})
    resume["language"] = detected_lang

    # Try to parse to structured JSON (optional, may fail if LLM not configured)
    try:
        processed_data = await parse_resume_to_json(markdown_content, language=detected_lang)
        db.update_resume(
            resume["resume_id"],
            {
...
async def improve_resume_preview_endpoint(
    ...
    language = resume.get("language") or get_content_language()
...
async def _improve_preview_flow(
    *,
    ...
    if not job_keywords or job_keywords_hash != content_hash:
        job_keywords = await extract_job_keywords(job["content"], language=language)
...
async def retry_processing(resume_id: str) -> ResumeUploadResponse:
    ...
    try:
        processed_data = await parse_resume_to_json(markdown_content, language=resume.get("language", "en"))
        db.update_resume(
            resume_id,
            {
                "processed_data": processed_data,
...
async def generate_cover_letter_endpoint(resume_id: str) -> GenerateContentResponse:
    ...
    # Use resume's detected language, fall back to user config
    language = resume.get("language") or get_content_language()
