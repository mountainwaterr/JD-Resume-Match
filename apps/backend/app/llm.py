...
LLM_TIMEOUT_JSON = 90  # single-call pipeline, no serial phases
...
def resolve_api_key(stored: dict, provider: str) -> str:
    ...
    default > ``DEFAULT_LLM_API_KEY`` env var — EXCEPT for providers in
    ``_PROVIDERS_WITHOUT_ENV_KEY_FALLBACK`` (``openai_compatible`` / ``ollama``),
    where the env-level default is skipped so a paid OpenAI key in
    ``LLM_API_KEY`` cannot leak to a local self-hosted server when the user
    leaves the provider key blank.
    ...
    if not api_key:
        ...
        # Ultimate fallback: default key for zero-config demo usage
        if not api_key and provider not in _PROVIDERS_WITHOUT_ENV_KEY_FALLBACK:
