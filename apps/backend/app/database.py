...
class Database:
    ...
    @property
    def jd_analyses(self) -> Table:
        """JD analysis results table."""
        return self.db.table("jd_analyses")
    ...
    def create_jd_analysis(
        ...
        """Create a new JD analysis result entry."""
        ...
        self.jd_analyses.insert(doc)
        return doc

    def get_jd_analysis(self, analysis_id: str) -> dict[str, Any] | None:
        """Get JD analysis by ID."""
        JDAnalysis = Query()
        result = self.jd_analyses.search(JDAnalysis.analysis_id == analysis_id)
        return result[0] if result else None

    def get_jd_analysis_by_hash(self, content_hash: str) -> dict[str, Any] | None:
        """Get JD analysis by content hash (for cache lookup)."""
        JDAnalysis = Query()
        result = self.jd_analyses.search(JDAnalysis.content_hash == content_hash)
        return result[0] if result else None

    def list_jd_analyses(self) -> list[dict[str, Any]]:
        """List all JD analyses."""
        return list(self.jd_analyses.all())
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_jd_analyses": len(self.jd_analyses),
