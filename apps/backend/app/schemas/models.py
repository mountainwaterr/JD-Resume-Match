...
class JobUploadRequest(BaseModel):
    ...
    job_descriptions: list[str] = Field(
        min_length=1, max_length=20, description="List of job description texts"
    )
    resume_id: str | None = Field(default=None, min_length=1, max_length=50)
    ...
    def _validate_job_descriptions(cls, value: list[str]) -> list[str]:
        for i, desc in enumerate(value):
            ...
            if len(desc.strip()) < 50:
                raise ValueError(f"job_descriptions[{i}] 过短（最少50个字符）")
            if len(desc) > 10000:
...
class ImproveResumeRequest(BaseModel):
    ...
    resume_id: str = Field(min_length=1, max_length=50)
    job_id: str = Field(min_length=1, max_length=50)
    prompt_id: str | None = Field(default=None, min_length=1, max_length=50)
...
class PromptConfigRequest(BaseModel):
    ...
    default_prompt_id: str | None = Field(default=None, max_length=50)
...
class FeaturePromptsRequest(BaseModel):
    ...
    cover_letter_prompt: str | None = Field(default=None, max_length=10000)
    outreach_message_prompt: str | None = Field(default=None, max_length=10000)
...
class ApiKeysUpdateRequest(BaseModel):
    ...
    openai: str | None = Field(default=None, max_length=500)
    anthropic: str | None = Field(default=None, max_length=500)
    google: str | None = Field(default=None, max_length=500)
    openrouter: str | None = Field(default=None, max_length=500)
    deepseek: str | None = Field(default=None, max_length=500)
    groq: str | None = Field(default=None, max_length=500)
...
class ResetDatabaseRequest(BaseModel):
    ...
    confirm: str | None = Field(default=None, max_length=50)

    @field_validator("confirm")
    @classmethod
    def _validate_confirm(cls, value: str | None) -> str | None:
        if value is not None and value != "RESET_ALL_DATA":
            raise ValueError('必须输入 "RESET_ALL_DATA" 以确认重置')
        return value
