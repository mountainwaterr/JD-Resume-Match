...
class Settings(BaseSettings):
    ...
    def get_effective_api_key(self) -> str:
        ...
        Priority: Environment/settings value > config.json > DEFAULT_LLM_API_KEY env var > empty string
        """
        if self.llm_api_key:
            return self.llm_api_key
        from_config = _get_llm_api_key_with_fallback()
        if from_config:
            return from_config
        # Ultimate fallback: default key for zero-config demo usage
        import os
        return os.environ.get("DEFAULT_LLM_API_KEY", "")
