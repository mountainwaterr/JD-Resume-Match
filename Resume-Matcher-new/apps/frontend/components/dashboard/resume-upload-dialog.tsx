  ...
  const [
    ...
    onUploadSuccess: (uploadedFile, response) => {
      ...
      if (data.resume_id) {
        ...
        const successMessage = data.is_master ? '简历已上传并设为主简历。' : '简历上传成功。';
        if (processingFailed) {
          // Keep dialog open on failure so users can retry processing.
          setUploadFeedback({
            type: 'error',
            message: 'AI 解析失败。请尝试其他文件，或关闭此对话框。',
          });
  ...
  const handleRetryProcessing = async () => {
    ...
    try {
      ...
      if (result.processing_status !== 'ready') {
        setUploadFeedback({ type: 'error', message: '重试失败。请尝试上传其他文件。' });
        return;
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        {trigger || (
          <Button className="rounded-lg border border-border-soft shadow-md hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all">
            <UploadIcon className="w-4 h-4 mr-2" />
            {'上传简历'}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="sm:max-w-md bg-background border border-border-soft shadow-xl p-0 gap-0 rounded-lg">
        <DialogHeader className="p-6 border-b border-border-soft bg-white">
        ...
        <div className="p-6 bg-background">
          <div
            className={`
                            ...
                            ${isDragging ? 'border-blue-700 bg-blue-50' : 'border-steel-grey hover:border-border-soft hover:bg-white'}
                            ${currentFile ? 'bg-white border-solid border-border-soft' : ''}
          ...
          >
            ...
            {isUploadingGlobal ? (
              <div className="flex flex-col items-center py-4">
                ...
                <p className="font-mono text-sm font-bold uppercase text-primary">{'上传中...'}</p>
              </div>
            ) : currentFile ? (
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 text-left overflow-hidden">
                  <div className="w-10 h-10 border border-border-soft bg-paper-tint flex items-center justify-center shrink-0">
              ...
              <div className="flex flex-col items-center py-4">
                <div className="w-12 h-12 border border-border-soft bg-white shadow-md flex items-center justify-center mb-4">
                  <UploadIcon className="w-6 h-6 text-black" />
                </div>
                <p className="font-bold text-lg mb-1">{'点击或拖拽文件'}</p>
        ...
        <div className="p-4 border-t border-border-soft bg-white flex justify-end gap-2">
          {uploadFeedback?.type === 'error' && failedResumeId && (
            <Button
              variant="outline"
              className="rounded-lg border-border-soft hover:bg-paper-tint"
          ...
          <DialogClose asChild>
            <Button variant="outline" className="rounded-lg border-border-soft hover:bg-paper-tint">
              {'取消'}
            </Button>
          </DialogClose>
        </div>
